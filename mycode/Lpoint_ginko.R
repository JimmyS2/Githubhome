...
# RC1 과 RC2의 평균값 산출
cor( ((pca2_varimax$scores[,1])*(pca2_varimax$scores[,2])/2),
     final$sales) 

cbind(pca2_none$scores[,1], pca2_varimax$scores[,1]) # score 값들이 거의 비슷한 것을 확인가능. 
cor(pca2_none$scores[,1], pca2_varimax$scores[,1]) # 상관계수도 거의 1에 가깝다

libstick <- cbind(final, 
                  (pca_varimax$scores[,1] + pca_varimax$scores[,2])/2) %>%
  as_tibble %>% rename('PREF_INDEX' = "(pca_varimax$scores[, 1] + pca_varimax$scores[, 2])/2") # PREF_INDEX가 바로 최종 '온라인 선호지수' 이다.
