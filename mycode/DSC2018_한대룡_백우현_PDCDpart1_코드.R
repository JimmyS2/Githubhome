...
##############################################################################
##############################################################################
normal_m<-normal[which(normal$SEX==1),]
bad_m<-bad[which(bad$SEX==1),]
person_m<-person[which(person$SEX==1),]
...
for ( w in c(1:10)){
  ...
  ################################### stop for a moment ######################################
  ####### male : m_prob_file, m_imp_matrix   //  female : f_prob_file, f_imp_matrix###########
  ############################################################################################
  #    prob_file 1 : 10 #
  nam<-paste0('m_prob_file',w , '.csv')
  nam2<-paste0('m_imp_matrix',w , '.csv')
...
## 10개 prob 파일 앙상블 ##
prob_file1<-read.csv('m_prob_file1.csv')
prob_file2<-read.csv('m_prob_file2.csv')
prob_file3<-read.csv('m_prob_file3.csv')
prob_file4<-read.csv('m_prob_file4.csv')
prob_file5<-read.csv('m_prob_file5.csv')
prob_file6<-read.csv('m_prob_file6.csv')
prob_file7<-read.csv('m_prob_file7.csv')
prob_file8<-read.csv('m_prob_file8.csv')
prob_file9<-read.csv('m_prob_file9.csv')
prob_file10<-read.csv('m_prob_file10.csv')
...
oneid<-which(prob_file_total$pred>=0.3)
...
m_CM<-confusionMatrix(prob_file_total$pred,as.factor(real_testdata$label))
m_CM
re<-m_CM$byClass[6]
pr<-m_CM$byClass[5]
f2<-(5*pr*re)/(4*pr+re)
...
imp_matrix1<-read.csv('m_imp_matrix1.csv')
imp_matrix2<-read.csv('m_imp_matrix2.csv')
imp_matrix3<-read.csv('m_imp_matrix3.csv')
imp_matrix4<-read.csv('m_imp_matrix4.csv')
imp_matrix5<-read.csv('m_imp_matrix5.csv')
imp_matrix6<-read.csv('m_imp_matrix6.csv')
imp_matrix7<-read.csv('m_imp_matrix7.csv')
imp_matrix8<-read.csv('m_imp_matrix8.csv')
imp_matrix9<-read.csv('m_imp_matrix9.csv')
imp_matrix10<-read.csv('m_imp_matrix10.csv')
...
one_female_imp<-importance_frame
one_female_imp_vec<-c(importance_frame[,1],importance_frame[,2],importance_frame[,3],importance_frame[,4],
                      importance_frame[,5],importance_frame[,6],importance_frame[,7],importance_frame[,8],
                      importance_frame[,9],importance_frame[,10])


importance_vector[which(table(one_female_imp_vec)==10)]
sort(table(one_female_imp_vec),decreasing = T)
