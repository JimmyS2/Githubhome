### 아모레퍼시픽 종가 예측하기 모델링
## 롤링윈도우 기법을 실행하기 위해서는 109~114번째 줄에 있는 코드를 실행하기
...
hidden_dim = 9      # 각 lstm 셀의 은닉층 출력 크기
...
# 데이터를 로딩
raw_data = np.loadtxt('AmorePacific_final.csv', delimiter=',' , skiprows=1)
...
weighted_views = raw_data[:,14]
norm_weighted_views = MinMaxScale(weighted_views)

#아모레퍼시픽 종가
close = raw_data[:,15]
norm_close = MinMaxScale(close) 
...
x = np.stack(( norm_kospi, norm_hansen, norm_nasdaq, norm_sanhai,norm_nikei,
              ...
              norm_subscribers,norm_weighted_views,
              norm_close ))
x = np.transpose(x)
...
trainY = MinMaxUnScale(close,trainY)
train_predict = MinMaxUnScale(close,train_predict)  
train_error = sess.run(rmse, feed_dict={targets: trainY, predictions: train_predict})
print('train_rmse:',train_error)

#Validation

validationY = MinMaxUnScale(close,validationY)
validation_predict = MinMaxUnScale(close,validation_predict)
validation_error = sess.run(rmse, feed_dict={targets: validationY, predictions: validation_predict})
...
testY = MinMaxUnScale(close,testY)
test_predict = MinMaxUnScale(close,test_predict)
test_error = sess.run(rmse, feed_dict={targets: testY, predictions: test_predict})
print('input_data_dim:', input_data_dim, end='')
print(',sequence_length:', sequence_length, end='')
print(',hidden_dim:', hidden_dim, end='')
